...
import com.hengaiw.model.TradeStatus;
import com.hengaiw.model.builder.AlipayTradeCancelRequestBuilder;
...
abstract class AbsAlipayTradeService extends AbsAlipayService implements AlipayTradeService {
    ...
    protected AlipayF2FPayResult checkQueryAndCancel(String outTradeNo, String appAuthToken, AlipayF2FPayResult result,
                                                   AlipayTradeQueryResponse queryResponse,int maxQueryRetry,int queryDuration) {
        ...
        AlipayTradeCancelResponse cancelResponse = cancelPayResult(builder,maxQueryRetry,queryDuration);
    ...
    // 轮询查询订单支付结果
    protected AlipayTradeQueryResponse loopQueryResult(AlipayTradeQueryRequestBuilder builder,int maxQueryRetry,int queryDuration) {
        AlipayTradeQueryResponse queryResult = null;
        for (int i = 0; i < maxQueryRetry; i++) {
            Utils.sleep(queryDuration);
    ...
    // 根据外部订单号outTradeNo撤销订单
    protected AlipayTradeCancelResponse cancelPayResult(AlipayTradeCancelRequestBuilder builder,int maxCancelRetry,int cancelDuration) {
        ...
        if (needRetry(response)) {
            ...
            asyncCancel(builder,maxCancelRetry,cancelDuration);
        }
        return response;
    }

    // 异步撤销
    protected void asyncCancel(final AlipayTradeCancelRequestBuilder builder,int maxCancelRetry,int cancelDuration) {
        executorService.submit(new Runnable() {
            @Override
            public void run() {
                for (int i = 0; i < maxCancelRetry; i++) {
                    Utils.sleep(cancelDuration);
