...
public class AlipayTradeServiceImpl extends AbsAlipayTradeService {
    ...
    public static class ClientBuilder {
        ...
        public AlipayTradeServiceImpl build(AlipayConfig config) {
            if (StringUtils.isEmpty(gatewayUrl)) {
                gatewayUrl = config.getOpenApiDomain(); // 与mcloudmonitor网关地址不同
            }
    ...
    public AlipayF2FPayResult tradePay(AlipayTradePayRequestBuilder builder,int maxQueryRetry,int queryDuration) {
        ...
        if (response != null && Constants.SUCCESS.equals(response.getCode())) {
            ...
            AlipayTradeQueryResponse loopQueryResponse = loopQueryResult(queryBuiler,maxQueryRetry,queryDuration);
            return checkQueryAndCancel(outTradeNo, appAuthToken, result, loopQueryResponse,maxQueryRetry,queryDuration);
            ...
            return checkQueryAndCancel(outTradeNo, appAuthToken, result, queryResponse,maxQueryRetry,queryDuration);
